...
def load_file_types(file_path):
    """功能：读取并解析一个JSON文件，该文件包含不同类型的文件扩展名及其描述。
        输入：file_path，一个字符串，表示JSON文件的路径。
        输出：字典，键是文件类型，值是包含扩展名和描述的字典列表。"""
...
def build_extension_mapping(file_types):
    """功能：将从JSON文件中读取的数据转换成一个扩展名到文件类型和描述的映射。
        输入：file_types，字典，包含从JSON文件中读取的文件类型和扩展名信息。
        输出：字典，键是文件扩展名（小写），值是一个包含文件类型和描述的字典。"""
    extension_to_type = {}
...
def classify_files(folder_path, extension_to_type):
    """功能：遍历指定文件夹中的所有文件，根据文件扩展名将文件归类到子文件夹中。
        输入：folder_path：一个字符串，表示要分类文件的文件夹路径。
            extension_to_type：一个字典，键是文件扩展名（小写），值是一个包含文件类型和描述的字典。
        输出：无直接输出，但函数会根据文件类型移动文件，并在控制台上打印移动操作的信息。"""
    ...
    for file in files:
        ...
        if os.path.isfile(source_path):
            ...
            if file_extension in extension_to_type:
                ...
                target_folder = os.path.join(folder_path, file_info["type"])
...
if __name__ == "__main__":
    ...
    classify_files(folder_path, extension_to_type)
